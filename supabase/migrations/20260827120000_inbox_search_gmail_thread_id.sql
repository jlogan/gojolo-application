-- Expose inbox_threads.gmail_thread_id from search_inbox_threads for Gmail deep links in the UI.
-- Changing a RETURNS TABLE shape requires dropping the existing function first.
DROP FUNCTION IF EXISTS public.search_inbox_threads(uuid, uuid, text, text, integer, integer, uuid, text, text);

CREATE FUNCTION public.search_inbox_threads(
  p_org_id uuid,
  p_user_id uuid,
    ...
    WHERE t.org_id = p_org_id
      ...
      AND (
        CASE COALESCE(NULLIF(BTRIM(p_gmail_label_mode), ''), '')
          ...
          WHEN 'no_custom_labels' THEN
            t.gmail_labels IS NULL OR cardinality(t.gmail_labels) = 0
          ELSE true
        END
      )
      AND (
