-- Notify Slack on new message (INSERT inbox_messages) as well as new thread.
-- Covers the case: refresh-email adds 1 message to an existing thread (0 new threads).

-- Extend log table to support both triggers.
ALTER TABLE public.slack_notification_log
  ADD COLUMN IF NOT EXISTS trigger_type text DEFAULT 'thread' CHECK (trigger_type IN ('thread', 'message'));
COMMENT ON COLUMN public.slack_notification_log.trigger_type IS 'Which trigger produced this log: thread (inbox_threads INSERT) or message (inbox_messages INSERT).';
