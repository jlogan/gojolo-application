-- All filter: include archived threads that still have draft messages.
-- Also expose has_draft on list rows for client-side threadMatchesFilter.
...
GRANT EXECUTE ON FUNCTION public.search_inbox_threads(uuid, uuid, text, text, integer, integer, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.search_inbox_threads(uuid, uuid, text, text, integer, integer, uuid) TO service_role;
