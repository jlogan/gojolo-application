...
import {
  ...
  Search, User, Link2, Pencil, Trash2, FileText, ChevronDown, Mailbox, Tag,
} from 'lucide-react'
...
type InboxThread = {
  ...
  has_draft?: boolean
  /** Gmail custom labels aggregated on thread (search_inbox_threads). */
...
function threadHasCustomGmailLabels(t: Pick<InboxThread, 'gmail_labels'>): boolean {
  return (t.gmail_labels?.length ?? 0) > 0
}

function resolveInboxThreadSearchParams(
  filter: InboxFilter,
  selectedGmailLabel: string | null,
  hasGmailAccounts: boolean,
): { p_filter: InboxFilter; p_gmail_label: string | null; p_gmail_label_mode: string | null } {
  if (!hasGmailAccounts) {
    return { p_filter: filter, p_gmail_label: null, p_gmail_label_mode: null }
  }
  if (filter === 'all') {
...
export default function Inbox() {
  ...
  /** When set (from /invoices/:id/send), a successful send updates invoice email metadata. */
  const invoiceInboxDraftContextRef = useRef<{ invoiceId: string; kind: InvoiceEmailKind; status: string } | null>(null)
  const pendingInvoiceDraftEditorRef = useRef<{ draftMessageId: string } | null>(null)
  ...
  const fetchThreadsPage = useCallback(async (
    ...
    try {
      ...
      const gmailSearch = resolveInboxThreadSearchParams(filter, selectedGmailLabel, imapAccounts.some(a => a.host?.toLowerCase().includes('gmail.com')))
      debugLog('fetchThreads', { event: 'START', orgId: currentOrg.id, userId, filter, mailboxFilterId, selectedGmailLabel, gmailSearch, pageSize, offset, append, query })
      ...
      try {
        ({ data, error } = await Promise.race([
          supabase.rpc('search_inbox_threads', {
            p_org_id: currentOrg.id,
            p_user_id: userId,
            p_filter: gmailSearch.p_filter,
            p_query: query || null,
            p_limit: pageSize,
            p_offset: offset,
            p_imap_account_id: mailboxFilterId,
            p_gmail_label: gmailSearch.p_gmail_label,
            p_gmail_label_mode: gmailSearch.p_gmail_label_mode,
          }),
  ...
  }, [currentOrg?.id, filter, mailboxFilterId, selectedGmailLabel, userId, debugLog, pageSize, searchQuery, refreshGmailLabelOptions, toast, imapAccounts])
  ...
  const maybeCleanupThreadAfterDraftDeletes = useCallback(async (threadId: string, deletedIds: Set<string>) => {
    ...
    if (threadRemoved) {
      if (invoiceId) invoiceInboxDraftContextRef.current = null
        ...
        if (session?.access_token) {
          try {
            ...
            if (result.messages?.length || deletedIds.size > 0 || !res.ok) {
              ...
              void maybeCleanupThreadAfterDraftDeletes(tid, deletedIds)
                ...
                const retry = () => fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${session.access_token}`, 'apikey': import.meta.env.VITE_SUPABASE_ANON_KEY }, body: JSON.stringify({ threadId: tid }) })
                  ...
                  .then(({ res, payload: r }) => {
                    ...
                    if (r.messages?.length || retryDeleted.size > 0 || !res.ok) {
                      ...
                      void maybeCleanupThreadAfterDraftDeletes(tid, retryDeleted)
  ...
  useEffect(() => {
    ...
    supabase.from('imap_accounts').select('id, email, label, addresses, host').eq('org_id', currentOrg.id).eq('is_active', true)
      .then(({ data }) => {
  ...
  useEffect(() => {
    ...
    if (!imapAccounts.some(a => a.host?.toLowerCase().includes('gmail.com'))) return
    if (selectedGmailLabel != null) return
    setSelectedGmailLabel(GMAIL_DEFAULT_INBOX_LABEL)
    fetchSelectedGmailLabelRef.current = GMAIL_DEFAULT_INBOX_LABEL
  }, [filter, imapAccounts, selectedGmailLabel])
  ...
  useEffect(() => {
    ...
    invoiceInboxDraftContextRef.current = {
      invoiceId: state.invoiceDraft.invoiceId,
      kind: state.invoiceDraft.kind,
      status: state.invoiceDraft.status,
    }
    if (state.invoiceDraft.openEditor) {
      pendingInvoiceDraftEditorRef.current = { draftMessageId: state.invoiceDraft.draftMessageId }
    }
    toast(invoiceInboxDraftToastMessage(state.invoiceDraft.kind, false))
    navigate(location.pathname, { replace: true, state: null })
  // eslint-disable-next-line react-hooks/exhaustive-deps -- run once when invoice send navigation state arrives
  }, [location.state])
  ...
  const hasGmailAccounts = useMemo(
    () => imapAccounts.some(a => a.host?.toLowerCase().includes('gmail.com')),
    [imapAccounts],
  )
  ...
  useEffect(() => {
    const pending = pendingInvoiceDraftEditorRef.current
    if (!pending || messagesLoading || !selectedThreadId) return
    const draft = messages.find((m) => m.id === pending.draftMessageId && m.is_draft)
    if (!draft) return
  ...
  const handleSendReply = async () => {
    ...
    const sentThreadId = selectedThreadId ?? (data.threadId as string | undefined) ?? null
    ...
    if (invoiceSendCtx && sentThreadId) {
      ...
      if (userId) {
        await supabase.from('inbox_thread_assignments').insert({ thread_id: sentThreadId, user_id: userId }).then(({ error: assignErr }) => {
          if (assignErr && assignErr.code !== '23505') console.warn('[Inbox] invoice assignment failed', assignErr.message)
        })
      }
    }
        ...
        <div className="flex items-center gap-2 overflow-x-auto">
          {FILTERS.map(f => (
            <button key={f.id} type="button" onClick={() => {
              ...
              if (f.id === 'all' && imapAccounts.some(a => a.host?.toLowerCase().includes('gmail.com'))) {
                setSelectedGmailLabel(GMAIL_DEFAULT_INBOX_LABEL)
                fetchSelectedGmailLabelRef.current = GMAIL_DEFAULT_INBOX_LABEL
              } else if (f.id !== 'all') {
                setSelectedGmailLabel(null)
      ...
      <div className="flex flex-1 min-h-0">
        ...
        <div className={`${selectedThreadId || replyMode === 'compose' ? 'hidden md:flex' : 'flex'} w-full md:w-80 lg:w-96 flex-col border-r border-border bg-surface-muted/20 shrink-0`}>
          ...
          <div className="border-b border-border">
            ...
            {filter === 'all' && hasGmailAccounts && (
              <div className="px-2 pb-2">
                <div className="flex items-center gap-1.5">
                  <Tag className={`w-3.5 h-3.5 shrink-0 ${selectedGmailLabel && selectedGmailLabel !== GMAIL_DEFAULT_INBOX_LABEL ? 'text-accent' : 'text-gray-400'}`} />
                  <select
                    value={selectedGmailLabel ?? GMAIL_DEFAULT_INBOX_LABEL}
                    onChange={e => {
                      const value = e.target.value
                      applyGmailLabelFilter(value === GMAIL_DEFAULT_INBOX_LABEL ? GMAIL_DEFAULT_INBOX_LABEL : value || null)
                    }}
                    title="Filter by Gmail label"
                    className={`w-full h-9 rounded border px-2 py-0 text-xs font-medium leading-none focus:outline-none focus:ring-1 focus:ring-accent truncate ${
                      selectedGmailLabel && selectedGmailLabel !== GMAIL_DEFAULT_INBOX_LABEL
